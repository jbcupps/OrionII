...
All inter-component communication inside OrionII flows through the `EventBus` trait in `src-tauri/src/orion/bus/`. Phase 1 default is `InMemoryBus` (tokio broadcast). ADR-003 adds the durable NATS JetStream product transport without changing subscriber call shape; the earlier Iggy adapter remains experimental.
...
**+** Durable broker transports are back-end changes in `bus/mod.rs`'s implementation, not rewrites. No caller depends on the concrete bus type — all callers depend on the `SharedBus = Arc<dyn EventBus>` type alias.
...
- Durable broker packaging / vendoring (ADR-003 ships NATS JetStream as the product sidecar; Iggy remains adapter work).
