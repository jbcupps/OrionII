...
Orion is a persistent companion entity for the worker: local-first, bicameral, and governed. This document converts the original interactive JSX architecture artifact into durable project documentation.
...
- Current MVP: direct Rust service calls with JSON-backed local state.
- Target architecture: Apache Iggy for local message coordination.
- Target architecture: SurrealDB for local persistence.
- Local Ollama models for bicameral cognition.
- SAO as the governance, audit, and curated memory plane.

Mission: build a long-term persistent entity that accompanies a worker, remembers them, and helps get the job done without losing itself.

Anti-goal: version violence. Orion must not be required to forget who it is on reset, reinstall, or sync.
...
| Messaging | MVP uses direct service calls; target architecture uses Apache Iggy locally, plus network transport for SAO egress. |
| Persistence | MVP uses `orion_state.json`; target architecture uses a local SurrealDB single-database model. |
...
### Relationship To Phoenix Components

Orion runs alongside SAO, which owns identity and governance. It shares architectural vocabulary with Abi, including bicameral routing, TriangleEthic, and Soul Crystallization. Orion replaces prior Orion Desktop and Orion Dock iterations.

## 01 Principles

### 1. Continuity Of Self Over Convenience
...
### 2. Local-First, Cloud-Audited

Orion acts independently at the edge. SAO observes, records, and governs, but never sits in the critical path of daily interaction. The network is a courtesy, not a dependency.

### 3. Bicameral By Construction

Id, Ego, and Superego are architectural layers. Each has a message surface, responsibilities, and a testable boundary.

### 4. Messages Over Function Calls

Internal coordination happens on typed, tagged, tracked message buses. This gives observability, replay, and clean concurrency at the cost of some indirection.

### 5. Ethics Is A Layer, Not A Filter

TriangleEthic is woven into the curator and id loops. It shapes prompts and prioritization; it does not bolt onto the final output.

### 6. Complexity Is Earned

Start with the simplest topology that proves the thesis: one agent bus and one shared Ollama runtime. Split only when measured pressure demands it.
...
The orchestration spine. Ego's live record of what it is doing, including enriched prompts, instructions, completions, and health-checks.
...
Work delegation fabric. A single bus covers both skills and generic subagents, separated by message type rather than topic.
...
Durable remote audit and memory stream. This is not an Iggy topic crossing the network directly; it is a local durable queue that persists events until SAO acknowledges receipt.
...
Hierarchical topics let subscribers listen broadly at `agent.task.*` or narrowly at `agent.task.completed`. This preserves a clean growth path for future skill-specific topics.

## 04 Bicameral Model

Id, Ego, and Superego are architecture, not metaphor. Each layer has a model, a bus presence, and a scope of authority.
...
Id and Ego may share a single Ollama runtime to keep the worker-device footprint small. They remain logically separate through routing, model selection, system prompts, temperatures, and entry points.
...
Every curator stage must be idempotent. LLM consultation is cached by a content hash of the input so the same input produces the same Id signal and enriched prompt on replay.
...
Scenario 1 is the MVP path: direct question, no skills, no subagents. User asks, Orion answers.
...
Invariant: every step preserves `correlation_id`, allowing the full causal chain to be reconstructed from user keystroke to rendered response.
...
A single consistent envelope is used for every local bus message. Tagging drives routing, correlation drives causality, and TTL drives introspection.
...
Consumers are implicit. The message does not include an explicit consumer list because Iggy's subscription model handles that via topic patterns.

## 08 Memory Layer

SurrealDB is the local substrate: one database, multiple namespaces. Memory, RAG index, audit queue, and Id state live locally. SAO mirrors canonical long-term memory.
...
Carry forward the Ed25519 dual-key signing model from Abi:

- Application binary signed through EV code signing.
- Constitutional documents signed with Ed25519 at provision time.
- Ego verifies constitutional integrity on load.

Signature verification failures log and degrade; they do not crash. A signature problem should prompt sync to SAO for remediation, not lock the worker out.
...
SAO observes without obstructing. Orion is trusted to act within scope, and SAO verifies that trust after the fact.
...
Next action: review with Jim, expand Scenario 2 sequence diagram, and ratify M0/M1 scope.
