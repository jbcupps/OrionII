...
pub enum SupervisorError {
    #[error("could not locate iggy-server binary; set ORIONII_IGGY_SERVER or place the binary on PATH")]
    BinaryMissing,
...
impl IggySupervisor {
    ...
    pub async fn start(port: u16, bus: SharedBus) -> Result<Self, SupervisorError> {
...
fn locate_binary() -> Result<PathBuf, SupervisorError> {
    ...
    if let Ok(path) = std::env::var("PATH") {
        ...
        for dir in path.split(separator) {
            for ext in if cfg!(windows) { &[".exe", ""][..] } else { &[""][..] } {
    ...
    tauri::async_runtime::spawn(async move {
        ...
        loop {
            ...
            if restart_history.len() > MAX_RESTARTS_PER_WINDOW {
                ...
                publish_broker_unstable(&bus).await;
                return;
            }
            ...
            match spawn_child(&binary, &data_dir, port).await {
                Ok(new_child) => {
                    if let Err(error) = wait_for_port(port, STARTUP_TIMEOUT).await {
                        eprintln!("[iggy-supervisor] restarted child failed wait_for_port: {error}");
                    } else {
...
async fn publish_broker_unstable(bus: &SharedBus) {
    ...
    if let Err(error) = bus.publish(env).await {
        eprintln!("[iggy-supervisor] failed to publish broker-unstable governance envelope: {error}");
    }
}
